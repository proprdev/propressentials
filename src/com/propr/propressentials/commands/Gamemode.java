...
public class Gamemode implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length == 0) {
                player.sendMessage(ChatColor.RED + Bukkit.getPluginCommand("gamemode").getUsage());
            ...
            if (guest != null) {
                switch (args[0].toLowerCase()) {
                    ...
                    case "0":
                        ...
                        sender.sendMessage(ChatColor.GREEN + "Set " + guest.getName() + "'s gamemode to survival");
                        break;
                    case "c":
                    case "creative":
                    case "1":
                        guest.setGameMode(GameMode.CREATIVE);
                        sender.sendMessage(ChatColor.GREEN + "Set " + guest.getName() + "'s to creative");
                        break;
                    case "a":
                    case "adventure":
                    case "2":
                        guest.setGameMode(GameMode.ADVENTURE);
                        sender.sendMessage(ChatColor.GREEN + "Set " + guest.getName() + "'s adventure");
                        break;
                    case "sp":
                    case "spectator":
                    case "3":
                        guest.setGameMode(GameMode.SPECTATOR);
                        sender.sendMessage(ChatColor.GREEN + "Set " + guest.getName() + "'s spectator");
                        break;
                }
            }
        }
